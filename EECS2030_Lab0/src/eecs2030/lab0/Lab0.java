...
public class Lab0 {
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		if((airTemp <= 0) && (windSpeed >= 5)) {
			windChillIndex = 13.12 + 0.6215 * airTemp - 11.37 * Math.pow(windSpeed, 0.16) + 0.3965 * airTemp * Math.pow(windSpeed, 0.16);
		}
		else if((airTemp <= 0) && (windSpeed > 0 && windSpeed < 5)) {
			windChillIndex = airTemp + ((-1.59 + 0.1345 * airTemp) / 5) * windSpeed;			
		}
	...
	public static int compareTo(Range r1, Range r2) {
		...
		if((r1.getMinimum() - r1.getMaximum()) == (r2.getMinimum() - r2.getMaximum())) {
			return 0;
		}
		else if((r1.getMinimum() - r1.getMaximum()) > (r2.getMinimum() - r2.getMaximum())) {
			return -1;
		}
		else {
			return 1;
	...
	public static char middleChar(String s) {
		...
		if(s.length() % 2 != 0 ) {
			...
			return s.charAt(((s.length() / 2) + 1) - 1);
		}
